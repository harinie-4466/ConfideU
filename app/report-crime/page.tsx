...
export default function CrimeReportForm() {
  ...
  const { user } = useAuth();
  const { addReport, addEvidenceToReport } = useCrimeReports();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create report
    // Upload files - Convert to Base64 for Backend Storage
    const uploadedEvidence = await Promise.all(uploadedFiles.map(async (fileObj) => {
      return new Promise<{
        ...
        reader.onloadend = () => {
          if (reader.result) {
            resolve({
              name: fileObj.name,
              type: fileObj.type,
              content: reader.result as string, // Base64 Content
              size: fileObj.size
            });
          } else {
            resolve(null);
        ...
        reader.readAsDataURL(fileObj.fileContent);
      });
    }));
    ...
    const generatedCaseId = await addReport({
      reportedBy: user.id,
      reportedByRole: user.role as 'citizen' | 'in-organisation',
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-card border border-border rounded-lg p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6">Your crime report has been received and will be reviewed by our team.</p>
          ...
          <p className="text-sm text-muted-foreground mb-6">
            You can use this Case ID to check the status of your report. We will maintain complete anonymity and confidentiality of your identity.
          </p>

          <div className="space-y-3">
            {user?.role === 'citizen' || user?.role === 'in-organisation' ? (
              <>
                <Button
      ...
      <header className="bg-card border-b border-border sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-foreground">Submit Crime Report</h1>
          </div>
        </div>
      </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={handleInputChange}
                  placeholder="Full name of person involved"
                  className="w-full"
                  required
                />
              </div>
